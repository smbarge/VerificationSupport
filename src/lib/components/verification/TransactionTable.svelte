...
let insertResponseError = '';
  ...
  if (t.status === 'success') {
    ...
    const result     = await checkApplied(seat, recheckType, subjects);
    isAlreadyApplied = result.applied;
  ...
  function closeModal() {
    ...
    modalData  = null;
    modalTxnId = '';
    isAlreadyApplied = false;  // ← add
  checkingApplied  = false;  // ← add
  matchedSubjects  = [];     // ← a
   inserting           = false;
  insertDone          = false;
  insertError         = '';
  insertingResponse   = false;
  insertResponseDone  = false;
  insertResponseError = '';
  }
...
async function handleInsertResponse(data: Record<string, any> | null) {
  if (!data) return;

  // ── Print exactly what is passed to saveResponseData ──
  // console.log('=== INSERT RESPONSE — Data being sent ===');
  // console.log('seat (udf2):', data.udf2);
  // console.log('payerName:', data.payerName);
  // console.log('payerEmail:', data.payerEmail);
  // console.log('payerMobile:', data.payerMobile);
  // console.log('clientTxnId:', data.clientTxnId);
  // console.log('sabpaisaTxnId:', data.sabpaisaTxnId);
  ...
  // console.log('transDate:', data.transDate);
  // console.log('udf1 (recheckType):', data.udf1);
  // console.log('udf3 (subjects):', data.udf3);
  // console.log('Full data object:', data);
  // console.log('==========================================');

  insertingResponse   = true;
  insertResponseError = '';
  insertResponseDone  = false;
  ...
  insertingResponse = false;
  if (result.error) insertResponseError = result.error;
  else              insertResponseDone  = true;
}

//insert all 
// async function handleInsert(data: Record<string, any> | null) {
//   if (!data) return;

//   //  const subjects = Array.isArray(data.udf3)
//   //   ? data.udf3
//   //   : String(data.udf3 ?? '').split(',').map((s: string) => s.trim()).filter(Boolean);

//   // ── Print exactly what is passed to addRecheckApplication ──
//   // console.log('=== INSERT RECHECK — Data being sent ===');
//   // console.log('seat (udf2):', data.udf2);
//   // console.log('recheckType (udf1):', data.udf1, '→',
//   //   data.udf1 == '1' ? 'Without Photocopy' :
//   //   data.udf1 == '2' ? 'With Photocopy' : 'Revaluation'
//   // );
//   // console.log('subjects (udf3):', subjects);
//   // console.log('clientTxnId:', data.clientTxnId);
//   // console.log('sabpaisaTxnId:', data.sabpaisaTxnId);
//   // console.log('Full data object:', data);
//   // console.log('=========================================');
...
async function handleInsert(data: Record<string, any> | null) {
  ...
  // ── Step 1: Save response first ──
  const responseResult = await insertResponse(data);
  if (responseResult.error) {
    inserting   = false;
    insertError = responseResult.error;
    return;
  }

  // ── Step 2: Insert recheck application ──
  const result = await insertTransaction(data);
...
>
    ...
    <!-- Modal Header -->
      <div style="background-color: #1a3a6b;" class="px-5 py-4 flex items-center justify-between flex-shrink-0">
        ...
        <div class="flex items-center gap-3">
          ...
          <!-- ── Insert button — only for SUCCESS ── -->
        {#if String(modalData?.status ?? '').toUpperCase() === 'SUCCESS' || String(modalData?.status ?? '').toUpperCase() === 'CAPTURED'}

  {#if checkingApplied}
    <span class="text-xs px-4 py-1.5 rounded-lg font-semibold text-white/70"
  ...
  {:else if isAlreadyApplied}
    <div class="flex items-center gap-2">
      ...
      <!-- ── Insert Response always shows ── -->
      {#if insertResponseDone}
      ...
      {:else}
        <button
          on:click={() => handleInsertResponse(modalData)}
          disabled={insertingResponse}
          class="text-xs px-4 py-2.5 rounded-lg font-semibold transition-all hover:opacity-90 active:scale-95 border-2 border-white text-white disabled:opacity-50"
          style="background-color: #2bbcb0;"
        >
          {insertingResponse ? 'Saving...' : '+ Insert Response'}
        </button>
        {#if insertResponseError}
          <span class="text-xs text-red-300">{insertResponseError}</span>
        {/if}
      {/if}
      ...
      {#if matchedSubjects.length > 0}
        <span class="text-xs text-green-300">
          {matchedSubjects.map(s => s.subj_name || s.subj_code).join(', ')}
        </span>
      {/if}
  ...
  {:else}
    <div class="flex items-center gap-2">
      ...
      <!-- ── Insert button only when NOT already applied ── -->
      {#if insertDone}
        <span class="text-xs px-4 py-1.5 rounded-lg font-semibold text-white border-2 border-white"
              style="background-color: #16a34a;">
          ✓ Inserted
      ...
      {:else}
        <button
          on:click={() => handleInsert(modalData)}
          disabled={inserting}
          class="text-xs px-6 py-2.5 rounded-lg font-semibold transition-all hover:opacity-90 active:scale-95 border-2 border-white text-white disabled:opacity-50"
          style="background-color: #16a34a;"
        >
          {inserting ? 'Inserting...' : '+ Insert'}
        </button>
        {#if insertError}
          <span class="text-xs text-red-300">{insertError}</span>
        {/if}
      {/if}
