...
export const addRecheckApplication = async ({
  ...
  try {
    ...
    console.log('pendingRecheckApplicationId is',pendingRecheckApplicationId)
    ...
    const divnCode = candidateResult.div;
...
ud.mobile_confirmation_id = vm.mobile_confirm_unique_id`;
    ...
    if (recheckApplicationId) {
      let updateQuery = `update pending_recheck_application set status='SUCCESS' where pending_recheck_application_id=$1`;
      let value = [pendingRecheckApplicationId];

      let { rows } = await db.query(updateQuery, value);
    }
    try {
      let sendSms = await sendSMS({
        mobileNo: mobileNo,
        recheck_type: verificationType,
        createdAt,
      });
    } catch (e) {}

    return { error:0, errorMsg:'', recheckApplication:[] };
  } catch (error) {
