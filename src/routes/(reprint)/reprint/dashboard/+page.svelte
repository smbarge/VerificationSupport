<script lang="ts">
  import TransactionTable from '$lib/components/reprint/ReprintTable.svelte';
  import { fetchTransactions } from '$lib/api/reprintapi';  
 ...
 async function search() {
  ...
  const seat  = isPhone ? null : input.toUpperCase();
  const phone = isPhone ? input : null;
  ...
  const data = await fetchTransactions(seat, phone);

  loading = false;
  if (data.error) { notFound = true; return; }
...
function GoBack() {
    ...
    <header style="background-color: #2bbcb0;" class="shadow-md">
      <div class="max-w-7xl mx-auto px-4 py-3 flex items-center gap-4">
        ...
        <div class="flex-1">
          ...
          <p class="text-white/75 text-xs mt-0.5 tracking-wider">Fee Transaction Reprint Portal</p>
        </div>
  ...
  <div class="bg-[#1a3a6b] py-2 px-4">
    <div class="max-w-7xl mx-auto flex items-center justify-between">
      <p class="text-blue-200 text-xs">Student Fee Payment Reprint System</p>
...
<div class="bg-white rounded-2xl shadow border border-gray-100 overflow-hidden">
        ...
        <div class="flex flex-wrap gap-3 items-end">
          ...
          <div class="flex flex-col gap-1.5 flex-1 min-w-[220px]">
            <label class="text-xs font-semibold text-gray-600">
              Seat / Phone Number <span class="text-red-500">*</span>
            </label>
            ...
            oninput={(e) => {
               ...
               seatInput = /^\d+$/.test(v) ? v : v.toUpperCase();
            }}
            onkeydown={(e) => e.key === 'Enter' && search()}
            type="text"
            placeholder="Enter seat number or 10-digit phone"
            maxlength="10"
    ...
    <div class="bg-red-50 border border-red-200 rounded-2xl p-8 text-center">
      ...
      <p class="text-red-600 font-semibold">No records found for <strong>{searchedSeat}</strong></p>
      <p class="text-red-400 text-sm mt-1">Please verify the seat number or phone number and try again.</p>
    </div>
    {/if}
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow p-14 text-center">
      ...
      <p class="text-gray-600 font-semibold">Enter a seat number or phone number to view all fee transactions</p>
